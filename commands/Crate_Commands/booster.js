module.exports = {
    ...
    //    if(message.deletable) message.delete()
        ...
        const BoosterRole = message.guild.roles.cache.find(r => r.name === "Server Booster");
        let hasPermission = false;
        if(BoosterRole){if(message.member.roles.cache.has(BoosterRole.id)) hasPermission = true;}
        ...
        if(hasPermission){
          ...
          const output = await DB.GetCash(message.author.id);
            ...
            const embed = new MessageEmbed()
                ...
                .setColor('RANDOM')
                .setAuthor(`You are not a server booster ${message.author.username}>`, 'https://cdn.discordapp.com/attachments/795772613229412372/797310165501607964/ohyes8.gif');
            return message.channel.send(noAccess).then((msg) => {

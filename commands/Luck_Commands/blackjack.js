...
module.exports = {
  ...
  async execute(message) {
    ...
    if (!bet) {
      return message
        .reply(`Invalid Syntax! ${prefix}hl <bet>`)
    ...
    if (bet < 1 || !Number.isInteger(Number(bet))) {
      if (bet && bet.toLowerCase().includes("k")) {
        ...
        if (
          !Number.isInteger(Number(givenKay * 1000)) ||
          isNaN(givenKay * 1000)
        ) {
    ...
    if (bet > cash) {
      return message.reply(
        `You only have ${formatNumber(cash)}. You cant bet more than that`
      );
    }
    if (bet > 5000000) {
      return message.reply(
        `You can't bet more than **${formatNumber(5000000)} cash** at a time.`
      );
    }
    if (bet < 1) {
      return message.reply(`You cant bet less than ${formatNumber(1)} cash`);
    }
    ...
    function score() {
      if (addCards("user") > 21) {
        ...
        // User has exactly 21
        return { result: false, emoji: "ok", message: "You win! You have 21!" };
      } else if (addCards("bot") === 21) {
        // Bot has exactly 21
        return {
          result: false,
    ...
    const gambed = async final => {
      ...
      if (status.constructor === Object) {
        ...
        if (bet > coinCheck.cash) {
          ...
          return message.reply(
            `You cant afford to pay your bet anymore. I took half your bet instead`
          );
        }
        let finalMsg = "";
        // Win
        if (status.result) {
          ...
          finalMsg += `\nYou won **${winnings.toLocaleString()}** cash. You now have ${formatNumber(
            newCash.cash
          )}.`;
        } else {
          // Tie
          if (status.result === null) {
            const newCash = await DB.GetCash(message.author.id);
            finalMsg += `Your cash hasn't changed! You have ${formatNumber(
              newCash.cash
            )} cash still.`;
          } else {
            // Loss
            ...
            const newCash = await DB.GetCash(message.author.id);
            finalMsg += `You lost **${Number(
              bet
            ).toLocaleString()}** cash. You now have ${formatNumber(
              newCash.cash
            ).toLocaleString()}.`;
          }
        }
